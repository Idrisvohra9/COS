
-- DROP SCHEMA
-- IF EXISTS COS_DB;
-- 	CREATE SCHEMA COS_DB COLLATE = utf8_general_ci;

-- DROP DATABASE COS_DB;

USE COS_DB;

-- TABLES FOR STORING AND RETREIVING USER CREDENTIALS:

CREATE TABLE IF NOT EXISTS NewUser(
	ID INT PRIMARY KEY AUTO_INCREMENT,
    COUNTRY VARCHAR(30),
    EMAIL VARCHAR(40),
    USERNAME VARCHAR(15),
    PASS VARCHAR(10)
);

CREATE TABLE IF NOT EXISTS ExistingUser(
	ID INT PRIMARY KEY AUTO_INCREMENT,
    USERNAME VARCHAR(15),
    PASS VARCHAR(10)
);

-- INSERT INTO EXISTINGUSER VALUES(1,"IdrisCOS","AdminCOS69");
-- INSERT INTO EXISTINGUSER VALUES(2,"AryanCOS","AdminCOS69");
-- INSERT INTO EXISTINGUSER VALUES(3,"JayCOS","AdminCOS69");

SELECT * FROM EXISTINGUSER;

-- TABLES FOR STORING THE DATE AND TIME OF USER LOGGING ON AND OFF:

CREATE TABLE IF NOT EXISTS LOGON(
	ID INT,
    DATE VARCHAR(12),
    TIME VARCHAR(6),
    FOREIGN KEY(ID) REFERENCES ExistingUser(ID)
);

CREATE TABLE IF NOT EXISTS LOGOFF(
	ID INT,
    TIME VARCHAR(6),
    FOREIGN KEY(ID) REFERENCES ExistingUser(ID)
);

-- DROP TABLE NewUser;
-- DROP TABLE ExistingUser;
-- DROP TABLE LOGON;
-- DROP TABLE LOGOFF;